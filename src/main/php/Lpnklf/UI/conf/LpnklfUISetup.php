...
class LpnklfUISetup {
	 ...
	 */
	public static function initialize( $appName = "lpnklf_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	private static function initializeCore(){
		...
		LpnklfConfig::getInstance()->setDbName("cose_lpnklf");
		
		LpnklfConfig::getInstance()->initialize();
		LpnklfConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");
	...
	private static function initializeUI( $appName ){
		...
		Logger::log("Logger lpnklf ui configurado!");
		...
		$rastyLoader->loadXml(
				...
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Lpnklf/UI/",
		...
		//SorteosUIConfig::getInstance()->initialize("LpnklfMetroLayout");
